...
pub trait StorageTripleMap<K1: FullEncode, K2: FullEncode, K3: FullEncode, V: FullCodec> {
	...
	fn storage_triple_map_final_key1<KArg1>(k1: KArg1) -> Vec<u8> where
		KArg1: EncodeLike<K1>,
	{
		let module_prefix_hashed = Twox128::hash(Self::module_prefix());
		let storage_prefix_hashed = Twox128::hash(Self::storage_prefix());
	...
	/// Generate the second part of key used in top storage.
	fn storage_triple_map_final_key2<KArg1, KArg2>(k1: KArg1, k2: KArg2) -> Vec<u8> where
		KArg1: EncodeLike<K1>,
        KArg2: EncodeLike<K2>,
    {        
		let module_prefix_hashed = Twox128::hash(Self::module_prefix());
	...
	fn storage_triple_map_final_key<KArg1, KArg2, KArg3>(k1: KArg1, k2: KArg2, k3: KArg3) -> Vec<u8> where
		KArg1: EncodeLike<K1>,
        KArg2: EncodeLike<K2>,
        KArg3: EncodeLike<K3>,
	{
		...
		let key1_hashed = k1.borrow().using_encoded(Self::Hasher1::hash);
        let key2_hashed = k2.borrow().using_encoded(Self::Hasher2::hash);
        let key3_hashed = k3.borrow().using_encoded(Self::Hasher3::hash);
		...
		final_key.extend_from_slice(key1_hashed.as_ref());
        final_key.extend_from_slice(key2_hashed.as_ref());
        final_key.extend_from_slice(key3_hashed.as_ref());

		final_key
	}
}

impl<K1, K2, K3, V, G> storage::StorageTripleMap<K1, K2, K3, V> for G where
	K1: FullEncode,
    K2: FullEncode,
    K3: FullEncode,
	V: FullCodec,
	G: StorageTripleMap<K1, K2, K3, V>,
{
	type Query = G::Query;

	fn hashed_key_for<KArg1, KArg2, KArg3>(k1: KArg1, k2: KArg2, k3: KArg3) -> Vec<u8> where
		KArg1: EncodeLike<K1>,
        KArg2: EncodeLike<K2>,
        KArg3: EncodeLike<K3>,
	{
		Self::storage_triple_map_final_key(k1, k2, k3)
	}

	fn contains_key<KArg1, KArg2, Karg3>(k1: KArg1, k2: KArg2, k3: Karg3) -> bool where
		KArg1: EncodeLike<K1>,
        KArg2: EncodeLike<K2>,
        Karg3: EncodeLike<K3>,
	{
		unhashed::exists(&Self::storage_triple_map_final_key(k1, k2, k3))
	}

	fn get<KArg1, KArg2, Karg3>(k1: KArg1, k2: KArg2, k3: Karg3) -> Self::Query where
		KArg1: EncodeLike<K1>,
        KArg2: EncodeLike<K2>,
        Karg3: EncodeLike<K3>,
	{
		G::from_optional_value_to_query(unhashed::get(&Self::storage_triple_map_final_key(k1, k2, k3)))
	}
	...
	where
        ...
        KArg3: EncodeLike<K3>,
    {
		unhashed::get(&Self::storage_triple_map_final_key(k1, k2, k3)).ok_or(())
	}

	fn take<KArg1, KArg2, Karg3>(k1: KArg1, k2: KArg2, k3: Karg3) -> Self::Query where
		KArg1: EncodeLike<K1>,
        KArg2: EncodeLike<K2>,
        Karg3: EncodeLike<K3>,
	{
		let final_key = Self::storage_triple_map_final_key(k1, k2, k3);
	...
	fn insert<KArg1, KArg2, KArg3, VArg>(k1: KArg1, k2: KArg2, k3: KArg3, val: VArg) where
		...
		VArg: EncodeLike<V>,
	{
		unhashed::put(&Self::storage_triple_map_final_key(k1, k2, k3), &val.borrow())
	}

	fn remove<KArg1, KArg2, KArg3>(k1: KArg1, k2: KArg2, k3: KArg3) where
		KArg1: EncodeLike<K1>,
        KArg2: EncodeLike<K2>,
        KArg3: EncodeLike<K3>,
	{
		unhashed::kill(&Self::storage_triple_map_final_key(k1, k2, k3))
	}

	fn remove_prefix<KArg1>(k1: KArg1) where KArg1: EncodeLike<K1> {
		unhashed::kill_prefix(Self::storage_triple_map_final_key1(k1).as_ref())
	}

	fn iter_prefix_values<KArg1>(k1: KArg1) -> storage::PrefixIterator<V> where
		KArg1: ?Sized + EncodeLike<K1>
	{
		let prefix = Self::storage_triple_map_final_key1(k1);
	...
	fn mutate<KArg1, KArg2, KArg3, R, F>(k1: KArg1, k2: KArg2, k3: KArg3, f: F) -> R where
		KArg1: EncodeLike<K1>,
        KArg2: EncodeLike<K2>,
        KArg3: EncodeLike<K3>,
		F: FnOnce(&mut Self::Query) -> R,
	{
		Self::try_mutate(k1, k2, k3, |v| Ok::<R, Never>(f(v))).expect("`Never` can not be constructed; qed")
	}
	...
	{
		Self::try_mutate_exists(k1, k2, k3, |v| Ok::<R, Never>(f(v))).expect("`Never` can not be constructed; qed")
	}

	fn try_mutate<KArg1, KArg2, KArg3, R, E, F>(k1: KArg1, k2: KArg2, k3: KArg3, f: F) -> Result<R, E> where
		KArg1: EncodeLike<K1>,
        KArg2: EncodeLike<K2>,
        KArg3: EncodeLike<K3>,
	...
	>(key1: KeyArg1, key2: KeyArg2, key3: KArg3) -> Option<V> {
		...
		unhashed::take(old_key.as_ref()).map(|value| {
			unhashed::put(Self::storage_triple_map_final_key(key1, key2, key3).as_ref(), &value);
			value
		})
	}
}
...
impl<
	...
	V: FullCodec,
	G: StorageTripleMap<K1, K2, K3, V>,
> storage::IterableStorageTripleMap<K1, K2, K3, V> for G where
    ...
    G::Hasher3: ReversibleStorageHasher,
{
	...
	fn iter_prefix(k1: impl EncodeLike<K1>) -> Self::PrefixIterator {
		...
		Self::PrefixIterator {
			...
			closure: |raw_key_without_prefix, mut raw_value| {
				let mut key_material = G::Hasher3::reverse(raw_key_without_prefix);
				Ok((K3::decode(&mut key_material)?, V::decode(&mut raw_value)?))
			},
		}
	}
	...
	fn iter() -> Self::Iterator {
		...
		Self::Iterator {
			...
			drain: false,
			closure: |raw_key_without_prefix, mut raw_value| {
				let mut k1_k2_k3_material = G::Hasher1::reverse(raw_key_without_prefix);
				let k1 = K1::decode(&mut k1_k2_k3_material)?;
				let mut k1_k2_material = G::Hasher2::reverse(k1_k2_k3_material);
                let k2 = K2::decode(&mut k1_k2_material)?;
                let mut k3_material = G::Hasher3::reverse(k1_k2_material);
                let k3 = K3::decode(&mut k3_material)?;
				Ok((k1, k2, k3, V::decode(&mut raw_value)?))
			},
		}
	}
	...
	fn translate<O: Decode, F: FnMut(K1, K2, K3, O) -> Option<V>>(mut f: F) {
		let prefix = G::prefix_hash();
			...
			.filter(|n| n.starts_with(&prefix))
		{
            ...
            let mut key3_material = G::Hasher3::reverse(&key2_material);
            let key3 = match K3::decode(&mut key3_material) {
                Ok(key3) => key3,
                Err(_) => {
                    log::error!("Invalid translate: fail to decode key3");
                    continue
                }
            };

			match f(key1, key2, key3, value) {
...
mod test_iterators {
	...
	use crate::{
		hash::StorageHasher,
		storage::{generator::StorageTripleMap, IterableStorageTripleMap, unhashed},
	};
	...
	crate::decl_storage! {
		trait Store for Module<T: Config> as Test {
			TripleMap: triple_map hasher(blake2_128_concat) u16, hasher(twox_64_concat) u32, hasher(twox_64_concat) u32 => u64;
		}
	}
