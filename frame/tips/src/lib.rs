...
use frame_support::traits::{
	Currency, Get, ExistenceRequirement::{KeepAlive}, OnUnbalanced,
	ReservableCurrency, SortedMembers, ContainsLengthBound
};
...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn report_awesome(origin: OriginFor<T>, reason: Vec<u8>, who: T::AccountId) -> DispatchResult {
		...
		pub fn tip_new(origin: OriginFor<T>, reason: Vec<u8>, who: T::AccountId, tip_value: BalanceOf<T>) -> DispatchResult {
		...
		pub fn tip(origin: OriginFor<T>, hash: T::Hash, tip_value: BalanceOf<T>) -> DispatchResult {
			let tipper = ensure_signed(origin)?;
			ensure!(T::Tippers::contains(&tipper), BadOrigin);
